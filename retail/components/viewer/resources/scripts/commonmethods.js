...
function validateDateValue(thisRef)
{
    ...
    var dateYear = timeSplit[0].split(dateSeperator);
    ...
    if (dateFormat == 1) {
        ...
        if(Dateparts[0] > daysInMonth[parseInt(Dateparts[1])])
            outputDate = currentdate;
        if (parseInt(Dateparts[1]) == 2 && parseInt(Dateparts[0]) >29 && dateYear[2] % 4 == 0)
            outputDate = currentdate;
    ...
    if(patternStringRef!=null && patternStringRef!=undefined && (dataType.toLowerCase() == 'text' || dataType == 'textarea'))
    {
       ...
       var validationType=document.getElementById(thisRef.id+"_validationString").innerHTML;
        ...
        if(! new RegExp(strPattern).test(value) || strPattern == "")
        {
           ...
           patternRef.innerText = ValidationMessage;
           toggleErrorTooltip(controlRef,msgRef,patternRef,false,1);
           return false;
        }
...
function validateMandatoryFields(frameid,isDoneClick)
{
    ...
    if(!TileMandatory(mandateControlInModal))
        return false;
    for(var i=0;i<Object.keys(ComponentValidatedMap).length;i++){
        let controlId=Object.keys(ComponentValidatedMap)[i];
	iscontrolvisible = isControlVisible($('#'+controlId)[0],listViewType,blankField,blankFieldControl);		
        if(!iscontrolvisible){
            console.log('control not visible>>'+controlId);
            mandatoryFieldCount--;
    ...
    try{
        var dummy=window.opener.applicationName;
        accessible=true;
    }catch(e){
    }  
    ...
    } else if( accessible && window.opener!=null && typeof window.opener.applicationName!='undefined' && (window.opener.applicationName==null || window.opener.applicationName=='')) {
...
function showUserDefinedMsg(controlId){
    ...
    if(patternRef!=null && patternRef!=undefined){
        if(customMsgJSON!=null && customMsgJSON!=undefined && customMsgJSON[controlId]!=undefined)
            patternRef.innerText = customMsgJSON[controlId];
    }
}
...
function getIformCustomMsg(controlId,key){
    var msg="";
    if(customMsgJSON!=null && customMsgJSON!=undefined ){
        if(customMsgJSON[controlId]!=undefined){
            ...
            key = "GENERAL_MAND_MSG";
            if(customMsgJSON[key]!=null && customMsgJSON[key]!=undefined){
                msg = customMsgJSON[key];
            }
        }
    }
