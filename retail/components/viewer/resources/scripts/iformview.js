    ...
    var listviewDataModified =false;
...
function applyFormattingGrid(controlId)
{   var ref = (typeof controlId == 'undefined' || controlId == null) ? ("") : ("#" + controlId);
    $(ref + ' .tabletextbox').each(function()
        {
        ...
        $(ref + ' .listviewlabel').each(function() {
...
function ctrOnchangeHandler(thisRef, op){
	...
	var validateMapKey=thisRef.id;//Bug 83970 Start
        ...
        if(valuesJSON["error"]=="false"){
            if(thisRef.classList.contains("advancedListviewControl")){
                listviewDataModified=true;
            }
        }
            ...
            if(isNaN(parseFloat(value)) || !isFinite(value) || (value.indexOf('.')>0 && (value.length-value.indexOf('.')-1)>(parseInt(thisRef.getAttribute("Precision"))))  ||(orgvalue.length>thisRef.getAttribute("floatlength"))){
                if(!(isNaN(parseFloat(value)) || !isFinite(value)) && (orgvalue.length>thisRef.getAttribute("floatlength"))){
                    ...
                    if(document.getElementById(tableId+"_"+thisRef.getAttribute("labelName")+"_msg")!=undefined)
                        document.getElementById(tableId+"_"+thisRef.getAttribute("labelName")+"_msg").innerText = correctValue;
                    thisRef.value="";
                    return false;
                } 
    ...
    function errorMsgHandler(){
        ...
        if(errorCode!=null){
            if (this.req.getResponseHeader("InvalidSession") === "Y" || this.req.getResponseHeader("Error") == "7006" || "11" === this.req.getResponseHeader("Error")) {
...
    }
    ...
    function customIFormHandler(op,dummySave)
    {
        ...
        if(isValid ){
         ...
         if( window.customValidation){  
             var customValResp = customValidation( op );
                if(typeof customValResp === 'boolean' && !customValResp ){
                    return "701";
                }
                else {
                    try {
                        customValResp = JSON.parse(decodeURIComponent(customValResp));
                    } catch (e) {
                    }
                    if (typeof customValResp != 'undefined' && typeof customValResp.isValid != 'undefined' && customValResp.isValid == 'false') {
                        var res = {"ResponseCode":"701"};
                        res["ResponseMessage"] = ( customValResp.ResponseMessage != undefined && customValResp.ResponseMessage != "")?customValResp.ResponseMessage:CUSTOM_VALIDATION_FAILED;
    ...
    function sanitize(value,type){        
        if(type=='int'){
            ...
            value=DOMPurify.sanitize(value,{ ADD_TAGS: ['script','iframe'], ADD_ATTR: ['formattedValue','onclick','maskingPattern','typeofvalue','Precision','onblur','onkeydown','onpaste','onfocus','onchange','datatype','labelName','allowSpaces','allowNumbers','SpecialCharacters','allowAlphabets','data-bs-dismiss','dataClass','autocomplete','dateformat','dateseparator','state','alphanum','pattern','inputmode','maskingpattern','custompattern','painted','combotype','maxlength','minvalue','maxvalue','maxdate','mindate','format','saveencrypt','ondblclick','controltype','floatlength','onkeypress','target','name','onmouseover'] });
        }else{
            value=DOMPurify.sanitize(value);  
        }      
        return value;
...
//                            jQuery(controlRef).autoNumeric('set', controlRef.innerHTML);
                ...
                try{
                   if(type=='input' && controlRef.value!=''){
                            jQuery(controlRef).autoNumeric('set', controlRef.value);
                            controlRef.title = controlRef.value; 
                        }
                        else if(type=='label' && controlRef.innerHTML){
                            jQuery(controlRef).autoNumeric('set', controlRef.innerHTML); 
                            controlRef.title = controlRef.innerHTML;
                        }
                        else if(type=='savedlabel' && controlRef.innerHTML){
                            jQuery(controlRef).autoNumeric('set', controlRef.innerHTML); 
                            controlRef.title = controlRef.innerHTML;
                        }
                } catch(ex){}
    ...
    function addRowToTable(controlId,isNext,isCopyRow){
        ...
        if(this.getAttribute("maskingPattern") && (this.getAttribute("maskingPattern").toString()==='currency_rupees' || this.getAttribute("maskingPattern").toString()==='currency_dollar' || this.getAttribute("maskingPattern").toString()==='currency_naira' || this.getAttribute("maskingPattern").toString()==='currency_yen' || this.getAttribute("maskingPattern").toString()==='currency_euro' || this.getAttribute("maskingPattern").toString()==='currency_french' || this.getAttribute("maskingPattern").toString()==='currency_greek' || this.getAttribute("maskingPattern").toString()==='currency_bahamas' || this.getAttribute("maskingPattern").toString()==='percentage'|| this.getAttribute("maskingPattern").toString()==='dgroup2'|| this.getAttribute("maskingPattern").toString()==='dgroup3'|| this.getAttribute("maskingPattern").toString()==='NZP'))
            ...
            dataValue[formatJSONValue(this.getAttribute("labelName").trim())]=formatJSONValue(value);
    ...
    function modifyTableRows(ref,controlId){
        ...
        if(autoIncrementLabelDisplay=='true')
        {
            ...
            value='{'+lineNo+'}'+value;
            document.getElementsByClassName('listViewHeader')[0].innerText=value;
        }
    }
    ...
    function modifyRowTableData(controlId){
        ...
        $(elementsArray).each(function(i) {
            var value;
            var labelName = this.getAttribute("labelName").trim();
            if((this.className.indexOf("denyNull")!=-1)&&(this.value==""||this.value==null)){
                nullElements.push(this.className.split("_")[1]);
            }
        ...
        if(this.getAttribute("maskingPattern") && (this.getAttribute("maskingPattern").toString()==='currency_rupees' || this.getAttribute("maskingPattern").toString()==='currency_dollar' || this.getAttribute("maskingPattern").toString()==='currency_naira' || this.getAttribute("maskingPattern").toString()==='currency_yen' || this.getAttribute("maskingPattern").toString()==='currency_euro' || this.getAttribute("maskingPattern").toString()==='currency_french' || this.getAttribute("maskingPattern").toString()==='currency_greek' || this.getAttribute("maskingPattern").toString()==='currency_bahamas' || this.getAttribute("maskingPattern").toString()==='percentage'|| this.getAttribute("maskingPattern").toString()==='dgroup2'|| this.getAttribute("maskingPattern").toString()==='dgroup3'|| this.getAttribute("maskingPattern").toString()==='NZP'))
            ...
            dataValue[formatJSONValue(labelName)]=formatJSONValue(value);
            ...
            if((this.getAttribute("datatype")==="text"||this.getAttribute("datatype")==="textarea"||this.getAttribute("datatype")==="Text")&&this.type==='text'||(this.type==='textarea' && !this.classList.contains("richtexteditor")))
            {
                ...
                if(charLim!=null&&charLim!=""&&value.length>charLim)
                    dataValue1[formatJSONValue(labelName)]=formatJSONValue(value.substring(0,charLim)+"...");
                else
                    dataValue1[formatJSONValue(labelName)]=formatJSONValue(value);
            }    
            else
                    dataValue1[formatJSONValue(labelName)]=formatJSONValue(value);
        });
                     ...
                     if(i>0){
                        if((mobileMode==="ios"||mobileMode==="android")&&this.children[0].getAttribute("columnMappedField")!=null){
                            if(this.children[0].getAttribute("columnMappedField")==="7"||this.children[0].getAttribute("columnMappedField")==="9"){
                                if(json[this.children[0].getAttribute("labelName").trim()]!=""){
                                    var newdate=new Date(json[this.children[0].getAttribute("labelName").trim()]);
                                    var ds="",df="";
                                    ...
                                    this.children[0].innerText=encode_ParamValue(df);
                                }
                                else
                                    this.children[0].innerHTML="";
                            }
                            else
                                this.children[0].innerText=encode_ParamValue(escapeStringForHTML(json[this.children[0].getAttribute("labelName").trim()]));
                        }
                        else
                            this.children[0].innerText=encode_ParamValue(escapeStringForHTML(json[this.children[0].getAttribute("labelName").trim()]));
    ...
    function modifyRowAdvancedlistviewResponseHandler(){
        ...
        try{
            ...
            $(elementToModify[0].parentNode).children().each(function(i) {
                if(i>0){
                    var labelValue = decode_utf8(json[this.children[0].getAttribute("labelName").trim()]);
                    var charLim="";
                    for(var k=1;k<charLimitData.length;k++)
                    {
                        if(charLimitData[k].innerHTML==this.children[0].getAttribute("labelName").trim())
                        {
                            if(charLimitData[k].getAttribute("charlimit")!=undefined)
                                charLim=charLimitData[k].getAttribute("charlimit");
                            break;
                    ...
                    if(labelValue!=null&&labelValue!=undefined&&labelValue!="undefined")  //Bug 91729
                        this.children[0].innerText=labelValue;
        ...
        if(listviewDataModified){
            ...
            var callback = function(result){
                if(result == true){  
                    ...
                    dataValue = saveRichTextEditorData('iFrameAdvancedListViewModal',dataValue);
                    var url = "action.jsp";
                    var requestString=  "controlId="+encode_utf8(controlId) +"&rowIndex="+rowIndex +"&dataValue=" + encode_utf8(JSON.stringify(dataValue)) + "&modifyFlag=yes&operation="+operation;  
                    var contentLoaderRef = new net.ContentLoader(url, nextPreviousOperation, ajaxFormErrorHandler, "POST", requestString, false);
                }else if(isBootboxCloseClicked==true){
                    isBootboxCloseClicked=false;
                }else{
                    this.modal('hide');
                    nextPreviousOperationHelper(controlId,rowIndex,operation);
                }    
                listviewDataModified=false;
            }
    ...
    function nextPreviousOperation(){
        var controlId = getQueryVariable(this.params, "controlId");
        var rowIndex = parseInt(getQueryVariable(this.params, "rowIndex"));
        var operation = getQueryVariable(this.params, "operation");
        var dataValue=decode_utf8(this.req.getResponseHeader("modifiedRowData")); //Bug 91541
        dataValue=sanitize(dataValue,'json');
        updateDataInMainGrid(dataValue,controlId);
        nextPreviousOperationHelper(controlId,rowIndex,operation);        
    }
    
    function updateDataInMainGrid(dataValue,controlId){
        var charLimitData=document.getElementById("scrollDiv_"+controlId).previousSibling.getElementsByTagName("th");
        var elementToModify=document.getElementsByClassName('advancedListviewrowToModify');
    ...
    function nextPreviousOperationHelper(controlId,rowIndex,operation){
...
function cellControlExistsInMap(tableId,rowIndex,columnName,cellData){
    ...
    function addRowlistviewResponseHandler(){
        ...
        if(crossState !=null ){
            crossState = crossState.getAttribute("state");
        }
        var rowhtml=sanitize("<table>"+this.req.responseText.trim()+"</table>",'html');
        var i;
    ...
    function attachDatePicker(){
        ...
        $(".mydatetimepicker").on('dp.show',function(e){
            if(this.classList.contains('tableinput')){
                $(this).parents('table').parent().css('overflow','visible');
            }
        })
        
        $(".mydatepicker").on('dp.show',function(e){
            if(this.classList.contains('tableinput')){
                $(this).parents('table').parent().css('overflow','visible');
            }
        })
        
        $(".mydatetimepicker").on('dp.hide',function(e){
            if(this.classList.contains('tableinput')){
                $(this).parents('table').parent().css('overflow','auto');
            }
        })
    ...
    function setStyle(controlId, attributeName, attributeValue,showHideAddDelete, showCheckBoxColumn, forcefullyEnable)
    {
            ...
            if( attributeValue.toLowerCase() == "true"){
                ...
                if(control.tagName!="TABLE" && !control.classList.contains("iform-radio")){
                    control.classList.add("disabledBGColor");
                        if(control.classList.contains("tilestyle")){
                            control.style.pointerEvents = "none";
                        }
                }
        ...
        else if(attributeName.toLowerCase() == "custompattern")
        {
            ...
            if (control != null)
            {
                ...
                document.getElementById(controlId + "_patternString").innerText = attributeValue;
            }
        }
    ...
    {
        document.getElementById("picklistHeader").innerText=sanitize(header);
		...
		$("#searchModal").find('.modal-dialog').removeClass('modal-lg');
        if(window.openPicklistInLargeModal){
            let largeModal= openPicklistInLargeModal(controlId);
            if(largeModal){
                $("#searchModal").find('.modal-dialog').addClass('modal-lg');
            }
        }
    ...
    function setPickListHeader(controlId,header){
         document.getElementById("picklistHeader").innerText=header;
    }
...
document.querySelector('#dynaimcManifest').setAttribute('href', manifestURL);
            ...
            if(sortOrder==undefined || (sortOrder==""||sortOrder=="A" || sortOrder=="N")){
                for(i=1;i<=getGridRowCount(tableId);i++){
                    for(j=1;j<=rows[i-1].getElementsByTagName("td").length;j++){
                        if(autoIncrementColumnsIndices[j]!=null && autoIncrementColumnsIndices[j]!=undefined)
                            if(batchCounter!=undefined && batchCounter!=""){
                                rows[i-1].getElementsByTagName("td")[autoIncrementColumnsIndices[j]].getElementsByClassName("control-class")[0].innerText = x;
                                x++;
                            }
                        else{
                            rows[i-1].getElementsByTagName("td")[autoIncrementColumnsIndices[j]].getElementsByClassName("control-class")[0].innerText = i;
                        }
                    }
                }
            }
                ...
                for(i=1;i<=getGridRowCount(tableId);i++){
                    for(j=1;j<=rows[getGridRowCount(tableId)-i].getElementsByTagName("td").length;j++){
                        if(autoIncrementColumnsIndices[j]!=null && autoIncrementColumnsIndices[j]!=undefined)
                            if(batchCounter!=undefined&& batchCounter!=""){
                            rows[getGridRowCount(tableId)-i].getElementsByTagName("td")[autoIncrementColumnsIndices[j]].getElementsByClassName("control-class")[0].innerText = x;
                            x++;
                            }
                        else{
                            rows[getGridRowCount(tableId)-i].getElementsByTagName("td")[autoIncrementColumnsIndices[j]].getElementsByClassName("control-class")[0].innerText = i;
                        }
                    }
                } 
            }
    ...
    function getWorkItemData(property){
        ...
        else if(property.toLowerCase() == "locale"){
            return iformLocale.replace("/",'');
        }
    }
    ...
    function setValues(jsonObj,sync){
        ...
        for(key in jsonObj){           
            ...
            if(control != null && control != undefined){
            if(control.classList.contains("iform-radio")){
                control = control.querySelector('input[type="radio"]');
            }
   ...
   function setColumnVisibleHelper(tableId,colIndex,flag)
    ...
    function setTableDataHelper(tableId,rowIndex,colIndex,cellData){
       ...
       if(table.getElementsByTagName("tr")[parseInt(rowIndex)+1]){
        ...
        {
            control.innerText=escapeStringForHTML(cellData);
        }
    ...
    function getValue(controlId)
    {
        ...
        if (control != null && control != undefined)
        {
            if (control != undefined && control.classList.contains("iform-radio")) {
                return (control.querySelector('input[type="radio"]:checked') != null)?control.querySelector('input[type="radio"]:checked').value : "" ; 
            }
            if($(control).hasClass("colorRange")){
    ...
    function getSelectedItemLabel(comboId)
    {
        ...
        if (combo != null && combo != undefined)
        {
            ...
            if (combo.type == "text") {
                ...
                var selectedText = combo.options[combo.selectedIndex].innerHTML;
    ...
    function addItemInRadio(radioId,value)
    {   
        try{
        value=sanitize(value,'html');    
        var radio = getControlElement(radioId);
        if(radio != null && radio != undefined)
           radio.innerHTML=$(value).find(".iform-radio")[0].innerHTML;
        }catch(ex){}
    }
    ...
    function selectSheet(tabID, index,clickActiveSheet) {
        if (document.getElementById(tabID) != null && document.getElementById(tabID) != undefined)
        {
            var links = document.getElementById(tabID).getElementsByTagName("a");
            var activeSheet=$(links[index]).hasClass('active');
            links[index].click();
            if(activeSheet && clickActiveSheet){
               onTabClick($(links[index]).attr('href').replace('#',''),$(links[index])[0],tabID,index);           
            }
        }
    }
    ...
    function getCheckedRadioRef(radioRef){
        return (radioRef.querySelector('input[type="radio"]:checked') != null)? radioRef.querySelector('input[type="radio"]:checked'):radioRef.querySelector('input[type="radio"]');
    }
   ...
   function getAutoCompleteDataHelper(jsonString,controlId,likeSearch,keyCount){
    ...
    function executeDBLinking(event,controlId,jsonData,isKeyDown){
     ...
     function populateComboValuesfromString(controlName,controlLabel,controlValue, isformLoad ){
        ...
        if( window.clearComboOnLoad){
            if(!window.clearComboOnLoad(controlName) ){
                clearComboOnLoad = false;
            }
        }
        if(clearComboOnLoad)
        ...
        if(window.setSingleValueAsSelected){
            if(setSingleValueAsSelected(controlName)){
...
function makeComboEditable(ref)
{
   ...
   function onTabClick(sheetID,ref,tabId,sheetindex,eventCall)
	...
	function setTableCellDataHelper(tableId,rowIndex,colIndex,cellData)
	{
		...
		var control = col.getElementsByClassName("control-class")[0];
                if( control == undefined ){
                    col.innerText = encode_ParamValue(cellData);
                }
	  try{	
            if(control.classList.contains('listviewlabel')){
                    if(control.getAttribute("maskingPattern")!=null && control.getAttribute("maskingPattern")!='undefined'
                        ...
                        if(control.getAttribute("custommasking")=="true"){
                            ...
                            control.innerText = encode_ParamValue(escapeStringForHTML(cellData));
		}
		else{
			if (jQuery(control).hasClass("radio-group") == true) {
                             var childElement = control.children;
...
//            }
    ...
    function setCustomMandatoryMsg(controlId,customMsg){
        ...
        if(controlMsgRef!=null && controlMsgRef!=undefined){
            controlMsgRef.innerText = customMsg;
        }
    }
...
function renderExecuteServerEventAPIData(outputArray){
    ...
    for (i = 0; i < outputArray.length; i++){
        ...
        else if(apiObj.API==="addItemInRadio"){
            addItemInRadio(apiObj.id,apiObj.value);
        }
        ...
        }else if(apiObj.API==="clearRadio"){
            clearRadio(apiObj.id);
        }	
    } 
    catch(ex){
...
function setCustomPattern(controlId,pattern)
{
    ...
    if(control !=null)
    {
        ...
        document.getElementById(controlId+"_patternString").innerText = pattern;
    }
}
...
function showCustomCtrlToolTip(ref){
    ...
    document.getElementById(id+"_msg").innerText=str;
...
function customMaskField(controlRef,param){
    ...
    if(controlRef!=null){
        ...
        if(val){
            ...
            if(controlRef.value != "" && controlRef.value!=undefined){
              controlRef.title = controlRef.value;
            }
           else{
