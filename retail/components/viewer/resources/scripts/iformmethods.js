function changeLocale(locale){
    ...
    if(response!=""){        
        ...
        var navigationHTML=res.navigationHTML;
        if(navigationHTML!=''){
            setNavigationBarHtml({'value':navigationHTML});
            var elem;
            if(document.getElementsByClassName("stepNavigationContainerParent")[0]!=null){
              elem = $($(".stepNavigationContainerParent .navigationProgressBar li")[0])[0];              
            }else if(document.getElementsByClassName("sideMenuNavigationBarParent")[0]!=null){
              elem=$($(".sideMenuNavigationBarParent  .sideMenuNavigationBar li")  [0])[0];
            }
            loadFragment(elem,0,undefined,iformLocale);            
        }else{
...
function reloadlocaleSpecicificFiles(res,prevLocale){
    ...
    var files = {"js":["constants.js","iformCustomMsg.js"],"css":["ifomstyle.css","floating-labels.css","bootstrap-rtl.min.css","froala"]};
    //reload JS files
