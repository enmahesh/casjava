...
function setSliderAttributes(id,attrib,value){
    ...
    if(attrib==="min"){        
        ...
        $("#"+id).parent().find(".leftRange").find("p")[0].innerText=value;        
    } else if(attrib==="max") {
        $(box).attr('max',value);
        slider.attr("max",value);   
        $("#"+id).parent().find(".rightRange").find("p")[0].innerText=value;
    }    
}
...
function setValue(controlName, controlValue) {
    ...
    var objComp = getControlElement(controlName);
    var i;
    if (objComp != null) {
        //Bug 84373 Start
	    ...
	    if(objComp.hasAttribute('autocompletevalue')&&controlValue===''){
            ...
            if ((objComp.type == 'text' || objComp.type == 'email') && objComp.classList.contains("form-input") && controlValue != '') {
                ...
                getControlElement(controlName + "_label").classList.add("input-focus-label");
            }
            if (objComp.type == "textarea" && objComp.classList.contains("form-textarea") && controlValue != '') {
                objComp.classList.add("input-focus");
                objComp.classList.add("textarea-focus");
                getControlElement(controlName + "_label").classList.add("input-focus-label");
                getControlElement(controlName + "_label").classList.add("textarea-focus-label");
            }
        ...
        {
            objComp.innerText = sanitize(controlValue);
        ...
        else if(objComp.type=='radio' || objComp.classList.contains('iform-radio'))
        {
            if(objComp.querySelector('input[type="radio"]:checked') != null){
                objComp.querySelector('input[type="radio"]:checked').parentElement.classList.remove("active");
                objComp.querySelector('input[type="radio"]:checked').checked = false;
            }
            if(objComp.querySelector("input[value='"+controlValue+"']") != null){
                objComp.querySelector("input[value='"+controlValue+"']").checked = true;
                objComp.querySelector("input[value='"+controlValue+"']").parentElement.classList.add("active");
            }		
        }
...
function navigHandler() {
    ...
    if (responseText != '') {
        ...
        if (json["status"] == "1") {
            ...
            if(extendSession == "Y")
                  url += "&ExtendSession=Y";
            if(typeof iformLocale != 'undefined'){
                url += "&locale="+encode_utf8(iformLocale.replace("/",''));
            }
...
function OTPHandler() {
    ...
    if (responseText != '') {
        ...
        if (json["status"] === "1") {
           ...
           didntgetotp.innerText=DIDNT_GET_OTP;
           ...
           otpSendLabel.innerText=sanitize(OPT_SEND_MSG+" "+userName,'string');
           ...
           function countdown() {
              if (timeLeft == 0) {
                ...
                elem.innerText=sanitize(OTP_EXPIRED+"<br>"+otpExpireMsg);
...
function loadFragment(ref, parentIndex, childIndex,changeLocale) {
    var url = "action_API.jsp";
    ...
    if(ref.classList.contains("active") && navigationClick !== "2" && !getDeviceType() && typeof changeLocale =='undefined'){
        return;
    }
        ...
        if (window.showSuccessMessageInApplication || window.showFailureMessageInApplication) {
            ...
            if(extendSession=="Y"){
             ...
             if(this.req.getResponseHeader("WorkitemStatus") == "true"){
                 ...
                 if (window.getCustomTimeoutOnPortalSubmit) {
                   timeoutInMills = getCustomTimeoutOnPortalSubmit();
                   showAlertDialog(msg, false, timeoutInMills);
                } else
...
function initialiseSlider(val, id) {
    ...
    sliderId.addEventListener('input', updateSliderRuntime);
...
function updateSliderRuntime(e){
    var slider = $(e.target);
    var val = slider.val();
    var max = slider.attr("max");
    var min = slider.attr("min");
    ...
    percentage*=100;
    slider.siblings(".colorRange")[0].style.width=percentage+"%";
     $($('#'+e.target.id).parents().eq("1").find(".textRangeValue")).val(val);
}
